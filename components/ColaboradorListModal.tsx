...
export default function ColaboradoresListModal({ visible, onClose, onSelectColaborador }) {
  ...
  const loadColaboradores = async () => {
    try {
      setLoading(true);
      const response = await Api.getAllBarbers();
      const colaboradorList = response.data || response || [];
      setClients(colaboradorList);
      setFilteredClients(colaboradorList);
    } catch (error) {
  ...
  const renderClient = ({ item, index }) => (
    ...
    >
      <ClientInfo>
        ...
        {item.loc && <ClientPhone> {item.loc}</ClientPhone>}
    ...
    <Container>
      ...
      {loading ? (
        <LoadingContainer>
          ...
          <Text style={{ marginTop: 12, color: '#666' }}>Carregando clientes...</Text>
        </LoadingContainer>
      ) : (
        <FlatList
          ref={flatListRef}
          ...
          ListEmptyComponent={
            <EmptyText>
              {searchText ? 'Nenhum colaborador encontrado' : 'Nenhum colaborador cadastrado'}
            </EmptyText>
          }
          ListFooterComponent={
            filteredClients.length > 0 ? (
              <Footer>
                <FooterText>
                  Total de colaboradores: {filteredClients.length}
                </FooterText>

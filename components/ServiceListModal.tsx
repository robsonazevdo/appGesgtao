...
export default function ServiceListModal({ visible, onClose, onSelectService }) {
  ...
  const loadClients = async () => {
    try {
      ...
      setClients(serviceList);
      setFilteredClients(serviceList);
    } catch (error) {
  ...
  const renderClient = ({ item, index }) => (
    ...
    >
      <ClientInfo>
        ...
        {item.phone && <ClientPhone>📞 {item.phone}</ClientPhone>}
        {item.email && <ClientEmail>✉️ {item.email}</ClientEmail>}
      </ClientInfo>
      <AgendButton onPress={() => handleAgendar(item)}>
        <AgendButtonText>Agendar</AgendButtonText>
      </AgendButton>
    </ClientCard>
  );
